...
public class CustomPWM extends PWMBase {
    ...
    public void setPwmRate(int rate) {
        execute(commands.get("setRate").replace("{rate}", String.valueOf(rate)).replace("{p}", String.valueOf(port)));
        pwmRate = rate;
    }

    @Override
    public void setPwmRange(int range) {
        execute(commands.get("setRange").replace("{range}", String.valueOf(range)).replace("{p}", String.valueOf(port)));
        pwmRange = range;
    }
